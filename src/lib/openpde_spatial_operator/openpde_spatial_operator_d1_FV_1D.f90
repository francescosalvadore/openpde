!< Concrete class of spatial operator of first derivative for Finite Volume 1D methods.
module openpde_spatial_operator_d1_FV_1D
    !< Concrete class of spatial operator of first derivative for Finite Volume 1D methods.
    ...
    type, extends(spatial_operator_d1) :: spatial_operator_d1_FV_1D
        !< Concrete class of spatial operator of first derivative for Finite Volume 1D methods.
        contains
            ! deferred public methods
            procedure :: operate !< Operator operation.
    endtype spatial_operator_d1_FV_1D
contains
    function operate(this, inp, dir) result(opr)
        !< Operator operation.
        class(spatial_operator_d1_FV_1D), intent(in)           :: this     !< The operator.
        ...
        opr_cur => associate_field_FV_1D(field_input=opr, emsg='calling procedure spatial_operator_d1_FV_1D%operate')
        inp_cur => associate_field_FV_1D(field_input=inp, emsg='calling procedure spatial_operator_d1_FV_1D%operate')
        mesh_cur => associate_mesh_FV_1D(mesh_input=inp%m, emsg='calling procedure spatial_operator_d1_FV_1D%operate')
        call opr_cur%associate_mesh(field_mesh=inp%m)
        do b=1, mesh_cur%nb
            do i=1, mesh_cur%blocks(b)%n
                opr_cur%blocks(b)%val(i) = (inp_cur%blocks(b)%val(i+1) - inp_cur%blocks(b)%val(i))/(mesh_cur%blocks(b)%h)
            enddo
        enddo
    end function operate
end module openpde_spatial_operator_d1_FV_1D
