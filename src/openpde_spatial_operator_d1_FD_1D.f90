!< Concrete class of spatial operator of first derivative for Finite Difference 1D methods.
module openpde_spatial_operator_d1_FD_1D
    !< Concrete class of spatial operator of first derivative for Finite Difference 1D methods.
    ...
    type, extends(spatial_operator_d1) :: spatial_operator_d1_FD_1D
        !< Concrete class of spatial operator of first derivative for Finite Difference 1D methods.
        contains
            ! deferred public methods
            procedure :: operate !< Operator operation.
    endtype spatial_operator_d1_FD_1D
contains
    function operate(this, inp, dir) result(opr)
        !< Operator operation.
        class(spatial_operator_d1_FD_1D), intent(in)           :: this     !< The operator.
        ...
        do i=1, mesh_cur%n
!            opr_cur%val(i) = (inp_cur%val(i+1) - inp_cur%val(i-1))/(2._R_P*mesh_cur%h)
            opr_cur%val(i) = (-1./12.*inp_cur%val(i+2) +2./3.*inp_cur%val(i+1) -2./3.*inp_cur%val(i-1) +1./12.*inp_cur%val(i-2))/(mesh_cur%h)
        enddo
    end function operate
end module openpde_spatial_operator_d1_FD_1D
